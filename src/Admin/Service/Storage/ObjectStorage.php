...
final class ObjectStorage
{
    ...
    private function buildUploadFailureMessage(FilesystemException $exception): string
    {
        ...
        if (str_contains($message, 'accessdenied') || str_contains($message, '403 forbidden')) {
            return 'No se pudo guardar el archivo en S3. Revisa los permisos IAM del usuario (s3:PutObject, s3:GetObject, s3:DeleteObject).';
        }

        if (str_contains($message, 'permanent redirect') || str_contains($message, 'correct region')) {
            return 'No se pudo guardar el archivo en S3. La región configurada en AWS_REGION no coincide con la del bucket.';
        }

        if (str_contains($message, 'nosuchbucket')) {
            return 'No se pudo guardar el archivo en S3. El bucket configurado en AWS_S3_BUCKET no existe o no es accesible.';
        }

        if (str_contains($message, 'invalidaccesskeyid') || str_contains($message, 'signaturedoesnotmatch')) {
            return 'No se pudo guardar el archivo en S3. Las credenciales AWS no son válidas.';
        }

        return 'No se pudo guardar el archivo en el almacenamiento. Verifica la configuración de AWS S3.';
    }
